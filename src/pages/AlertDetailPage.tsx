    ...
    <div className="space-y-6 pb-10">
      ...
      <section className="rounded-2xl border border-border/90 bg-card p-5 md:p-6 shadow-elevated">
        <div>
          ...
          <p className="mt-1 text-base text-slate-500">탐지된 이상 징후의 원인과 대응 조치를 확인합니다.</p>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-border/90 bg-card p-6 border-l-4 border-l-red-500 shadow-elevated">
        ...
        <h3 className="text-xl font-bold text-slate-900">강남역점 결제 이상 탐지</h3>
        <div className="mt-3 flex items-center gap-4 text-sm text-slate-500 font-medium">
            ...
            {causes.map((cause) => (
              <div key={cause.rank} className="space-y-2">
                <div className="flex justify-between text-sm font-medium">
                  <span className="text-slate-700">{cause.rank}순위: {cause.title}</span>
        ...
        <article className="rounded-2xl border border-border/90 bg-card p-6 shadow-elevated">
          ...
          <ul className="space-y-3">
            {actions.map((action, idx) => (
              <li key={idx} className="flex items-start gap-3 rounded-xl border border-slate-100 bg-[#F8FAFF] p-3 text-sm font-medium text-slate-700">
                <span className="text-primary font-bold">{idx + 1}</span>
                {action}
              </li>
            ))}
          </ul>
          <button className="w-full mt-6 bg-primary text-white py-2.5 rounded-lg text-sm font-bold shadow-md hover:bg-[#1E5BE9]">
            조치 완료 보고
          </button>
        </article>
      </div>
      ...
      <section className="rounded-2xl border border-border/90 bg-card p-6 shadow-elevated">
        <h4 className="text-lg font-bold text-slate-900 mb-6">처리 타임라인</h4>
        <div className="space-y-0 pl-1">
          {timeline.map((event, idx) => (
            <div key={idx} className="flex gap-4">
              <div className="flex flex-col items-center">
                <div className={cn("h-3 w-3 rounded-full border-2", event.done ? "bg-primary border-primary" : "bg-white border-slate-200")} />
                {idx < timeline.length - 1 && <div className="w-px flex-1 bg-slate-100 my-1" />}
              </div>
              <div className={`pb-6 ${idx === timeline.length - 1 ? "pb-0" : ""}`}>
                <p className={cn("text-sm font-medium", event.done ? "text-slate-900" : "text-slate-400")}>{event.desc}</p>
                <p className="text-[11px] font-bold text-slate-400 mt-1">{event.time}</p>
              </div>
            </div>
          ))}
        </div>
