...
const tone = (value: string) => {
  if (value === "위험" || value === "높음") return "border border-red-200 bg-card text-red-600";
  if (value === "주의" || value === "보통") return "border border-amber-200 bg-card text-amber-600";
  return "border border-[#d5deec] bg-card text-[#4a5568]";
};
  ...
  return (
    <>
      <div className="space-y-6">
        {/* Morning Briefing */}
        <section className="rounded-2xl border border-[#b8ccff] bg-[#eef3ff] p-5 md:p-6">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-0.5 text-sm font-bold text-foreground">서울 남부구역 2026-03-08 07:00 기준</p>
              </div>
            </div>
          </div>
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            <div className="rounded-xl border border-[#d5deec] bg-card p-3">
              <p className="text-xs font-medium text-muted-foreground">구역 전일 실적</p>
              <p className="mt-1 text-sm font-bold text-foreground">총 매출 6,470천원</p>
              <p className="text-xs text-red-500">구역 평균 -5.8% · 하락 매장 3곳</p>
            </div>
            <div className="rounded-xl border border-red-200 bg-red-50 p-3">
              <p className="text-xs font-medium text-red-700">오늘 주요 이슈</p>
              <p className="mt-1 text-sm font-bold text-foreground">A매장 이상 결제 탐지</p>
            ...
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-3">
              ...
              <p className="mt-1 text-sm font-bold text-foreground">A/B매장 미이행 2건</p>
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <div className="mb-2 flex items-center gap-2">
                <span className="rounded-full border border-[#d5deec] bg-[#f4f7ff] px-3 py-1 text-xs font-semibold text-primary">
                  서울 남부구역
                </span>
                <span className="text-sm text-muted-foreground">관리 매장: 총 {rows.length}개</span>
              </div>
              <h2 className="text-2xl font-bold text-foreground">수퍼바이저 보드</h2>
            ...
            <div className="flex items-center gap-3 rounded-xl border border-[#d5deec] bg-[#f4f7ff] px-4 py-3">
              ...
              <div>
                <p className="text-sm font-semibold text-foreground">고위험 매장 {stats.danger}곳</p>
                <p className="text-xs text-muted-foreground">긴급 현장 방문이 필요합니다</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="grid gap-4 md:grid-cols-3">
          ...
          {[
            ...
            >
              ...
              <p className="mt-2 text-3xl font-bold text-foreground">{s.count}개</p>
        ...
        <section className="grid gap-4 md:grid-cols-3">
          <article className="rounded-2xl border border-border/90 bg-card p-5 shadow-sm">
            <div className="flex items-center justify-between">
              <p className="text-sm font-medium text-muted-foreground">구역 평균 매출 증감</p>
              <Activity className="h-4 w-4 text-[var(--subtle-foreground)]" />
            </div>
            <p className="mt-2 text-2xl font-bold text-foreground">{avgDelta}%</p>
            <p className="mt-1 text-xs text-muted-foreground">전월 동기 대비</p>
            <div className="mt-3">
          ...
          <article className="rounded-2xl border border-border/90 bg-card p-5 shadow-sm">
            <div className="flex items-center justify-between">
              <p className="text-sm font-medium text-muted-foreground">이탈 징후 고객 수</p>
              <Activity className="h-4 w-4 text-[var(--subtle-foreground)]" />
            </div>
            <p className="mt-2 text-2xl font-bold text-foreground">1,240명</p>
          ...
          <article className="rounded-2xl border border-[#d5deec] bg-[#f4f7ff] p-5 shadow-sm">
            <div className="flex items-center justify-between">
              <p className="text-sm font-medium text-muted-foreground">AI 추천 금주 방문지</p>
              <MapPin className="h-4 w-4 text-primary" />
            </div>
            <p className="mt-2 text-xl font-bold text-foreground">{topDangerStores}</p>
            <p className="mt-1 text-xs text-muted-foreground">위험도 스코어 70점 이상</p>
        ...
        <section className="rounded-2xl border border-border/90 bg-card p-5 md:p-6">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-bold text-foreground">공지 이행 현황</h3>
          ...
          <div className="mt-4 space-y-3">
            {complianceData.map((item) => (
              <div key={item.store} className="flex items-center gap-3">
                <span className="w-14 shrink-0 text-sm font-medium text-[#34415b]">{item.store}</span>
                <div className="flex-1">
                  <div className="h-2 w-full overflow-hidden rounded-full bg-[#d5deec]">
                    <div
                ...
                <span className="w-14 shrink-0 text-right text-xs text-muted-foreground">{item.done}/{item.total}</span>
        ...
        <section className="rounded-2xl border border-border/90 bg-card p-5 md:p-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-lg font-bold text-foreground">담당 매장 KPI 비교</h3>
            </div>
            <select
              value={selectedKpi}
              onChange={(e) => setSelectedKpi(e.target.value as KpiMetric)}
              className="h-9 rounded-lg border border-[#d5deec] bg-card px-3 text-sm text-[#34415b]"
            >
              {kpiOptions.map((o) => (
          ...
          <p className="mt-1 text-sm text-muted-foreground">지표: {kpiOpt.label} ({kpiOpt.unit})</p>

          <div className="mt-5 space-y-3">
            {rankedRows.map((row, idx) => (
              <div key={row.id} className="flex items-center gap-3">
                <span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-bold ${
                  idx === 0 ? "bg-primary text-white" : "border border-[#d5deec] bg-card text-muted-foreground"
                }`}>
                  {idx + 1}
                </span>
                <span className="w-16 shrink-0 text-sm font-medium text-[#34415b]">{row.name}</span>
                <div className="flex-1">
                  <div className="h-2 w-full overflow-hidden rounded-full bg-[#d5deec]">
                    <div
        ...
        {/* 격차 분석 */}
        <section className="rounded-2xl border border-border/90 bg-card p-5 md:p-6">
          <h3 className="text-lg font-bold text-foreground">상위·하위 매장 격차 분석</h3>
          <p className="mt-0.5 text-sm text-muted-foreground">상위 1위 vs 하위 1위 기준 주요 지표 격차입니다.</p>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            {kpiOptions.slice(0, 3).map((opt) => {
              ...
              return (
                <div key={opt.value} className="rounded-xl border border-[#d5deec] bg-[#f4f7ff] p-3">
                  <p className="text-xs font-medium text-muted-foreground">{opt.label} 격차</p>
          ...
          <div className="mt-4 rounded-xl border border-[#d5deec] bg-[#f4f7ff] p-4">
            <p className="text-xs font-semibold text-[#34415b]">AI 원인 해석</p>
        ...
        <section className="rounded-2xl border border-border/90 bg-card p-5 md:p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-lg font-bold text-foreground">매장별 위험도 분석</h3>
              <p className="mt-0.5 text-sm text-muted-foreground">매출·마진·이탈·방문 주기를 종합한 우선순위입니다</p>
            </div>
              ...
              <tbody>
                {rows.map((row) => (
                  <tr key={row.id} className="border-t border-border">
                    ...
                    <td className="px-3 py-3">
                      <span className={`text-sm ${parseInt(row.lastVisit) > 30 ? "text-red-500 font-medium" : "text-muted-foreground"}`}>
                        {row.lastVisit}
                      </span>
                    </td>
                    <td className="px-3 py-3 text-right">
                      <div className="flex justify-end gap-2">
                        <button onClick={() => setDetailId(row.id)} className="rounded border border-border px-2 py-1 text-xs text-[#34415b]">상세</button>
      ...
      {detail && (
        <div className="fixed inset-0 z-40 grid place-items-center bg-slate-900/45 p-4">
          <div className="w-full max-w-lg rounded-2xl border border-border bg-card p-5 shadow-xl">
            <h4 className="text-lg font-bold text-foreground">매장 상세</h4>
            <p className="mt-3 text-sm text-[#34415b]">{detail.name}</p>
      ...
      {report && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-900/45 p-4">
          <div className="w-full max-w-md rounded-2xl border border-border bg-card p-5 shadow-xl">
            <h4 className="text-lg font-bold text-foreground">방문 리포트 — {report.name}</h4>
            <div className="mt-4 space-y-3">
              {[
                ...
                <div key={item.label} className="rounded-xl border border-[#d5deec] bg-[#f4f7ff] p-3">
                  <p className="text-xs font-semibold text-[#4a5568]">{item.label}</p>
                  <p className="mt-0.5 text-sm text-[#1a2138]">{item.value}</p>
                </div>
              ))}
            </div>
